...
public unsafe class GameIntegration : IDisposable {
    ...
    private Vector2? GetQuestLocation(OpenMapInfo* mapInfo) {
        ...
        if (focusLevel is not null) {
            ...
            if (LuminaCache<Quest>.Instance.FirstOrDefault(quest => 
                        MatchQuestName(quest, mapInfo) && 
                        quest is {IssuerLocation.Row: not 0 }) is { IssuerLocation.Value: { } issuerLocation, JournalGenre.Value: { } journalInfo }) {
    ...
    private void OnShowHook(AgentInterface* agent, bool a1, bool a2)
        => Safety.ExecuteSafe(() => {
#if DEBUG
    ...
    private void SetFlagMarker(AgentMap* agent, uint territoryId, uint mapId, float mapX, float mapY, uint iconId) 
        => Safety.ExecuteSafe(() => {
