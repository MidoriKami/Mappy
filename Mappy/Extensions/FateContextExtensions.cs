...
public static unsafe class FateContextExtensions {
    public static Vector4 GetColor(FateContext* context, float alpha = 0.33f) {
    ...
    public static TimeSpan GetTimeRemaining(FateContext* context) {
        if (context->Duration is 0) return TimeSpan.Zero;
        
        return TimeSpan.FromSeconds(context->StartTimeEpoch + context->Duration - DateTimeOffset.Now.ToUnixTimeSeconds());
    }
}