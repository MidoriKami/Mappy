...
public static unsafe class QuestHelpers {
    public static IEnumerable<Level>? GetActiveLevelsForQuest(string questName, uint mapId) => 
        (from quest in GetAcceptedQuests()
            let luminaData = Service.DataManager.GetExcelSheet<CustomQuestSheet>()?.GetRow(quest.QuestId + 65536u)!
    ...
    private static IEnumerable<Level> GetActiveLevelsForQuest(QuestWork quest, uint? madId = null) {
        var luminaQuest = Service.DataManager.GetExcelSheet<CustomQuestSheet>()?.GetRow(quest.QuestId + 65536u)!;
        ...
        return 
            ...
            select Service.DataManager.GetExcelSheet<Level>()?.GetRow(luminaQuest.ToDoLocation[index, levelRow].Row) into level // Get each of the 8 levels
